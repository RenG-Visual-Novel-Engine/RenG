...
/*
#cgo LDFLAGS: -L./lib -lSDL2 -lSDL2_mixer
...
func PlayingMusicChannel(channel int) bool {
	if int(C.Mix_Playing(C.int(channel))) == 0 {
		return false
	}
	return true
}
...
func FreeMusic(music *Mix_Music) {
	C.Mix_FreeMusic((*C.Mix_Music)(music))
}

func FreeChunk(chunk *Mix_Chunk) {
